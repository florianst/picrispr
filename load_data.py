...
def seqToOneHot(sequence, requireLength, seq_guide=None, mismatch_type=False):
...
def turnToOneHot(experiment, howMany=2, seqToOneHot=seqToOneHot, encodingFunction="", numBpWise=0): # takes the first two elements in each row of experiment and turns them into one-hot vectors sequentially
    requireLength = 23
    ...
    if encodingFunction in ["oneHotSingleNuclTargetMismatch", "oneHotDinuclTargetMismatch", "oneHotSingleNuclTargetMismatchType", "oneHotDinuclTargetMismatchType"]: # target-mismatch or target-mismatch type encodings
        for key, (sequence_target, sequence_guide) in enumerate(zip([experiment[i][0] for i in range(len(experiment))], [experiment[i][1] for i in range(len(experiment))])):
            onehot = seqToOneHot(sequence_target, requireLength, sequence_guide, encodingFunction in ["oneHotSingleNuclTargetMismatchType", "oneHotDinuclTargetMismatchType"])
        ...
        for key, sequence in enumerate([experiment[i][0] for i in range(len(experiment))]):
...
def normaliseCF(cleavage_freq, cutoff=1e-5, variance=2.0, mean=0.0, doClip=True): # normalises the cleavage frequencies given as a 1D list to a Gaussian N(0,variance^2) using a Box-Cox transformation, clipped at -variance^2 and variance^2
    ...
    cleavage_freq = cleavage_freq / np.sqrt(1/variancesq)  # adjust variance such that values >variancesq are more than one sigma above mean
    if doClip: cleavage_freq = np.clip(cleavage_freq, -variancesq+mean, variancesq+mean) # cap distribution (without discarding values)
