...
class CSVDataset():
    ...
    def getDataMatrix(self, dbFields, featureEncoding, CFtoScore, chooseSpecies="", booleanX=False, cutoff=1e-5, ext=False, featurenames=[], mode="torch", numBpWise=0, 
                      test_size=0.2, makeSparse=True, doSplit=True, filenameAppendix="", debug_mode=False):
        ...
        for i in range(num_experiments+1):
            ...
            if len(experiment) > 1:
                ...
                experiment, augmented, encoding, pt = featureEncoding(experiment, augmented, cutoff, CFtoScore, count=i, i=i+1, featurenames=featurenames, numBpWise=numBpWise)
...
class FeatureEncoding():
    def __init__(self, epiDim, epiStart, seqDim, featurenames=['A', 'T', 'C', 'G', 'CTCF', 'DNase', 'RRBS', 'H3K4me3', 'DRIP', 'energy1', 'energy2', 'energy3', 'energy4', 'energy5'], encodingFunction=""):
...
class TrainResult():
    def __init__(self, mode, regression, home, filenameAppendix, model, lr, dataPortions=None, final_testloss=None, tpr=[], fpr=[], precision=[], recall=[], aucs_roc=None, aucs_roc_std=None, aucs_prc=None, aucs_prc_std=None, spearmanrs=None, spearmanr_stds=None, pearsonrs=None, pearsonr_stds=None, train_set=None, explain_set=None, meta_train=None, meta_explain=None, encoding=None, siamese=None, interfaceMode=False, exp_ids=None, mismatchType=False, numBpWise=0):
        ...
        self.encoding, self.siamese, self.interfaceMode, self.mismatchType = encoding, siamese, interfaceMode, mismatchType
    ...
    def load(self, trainPath, mode="torch", filenameAppendix="", resultAppend="", device="cpu"):
        ...
        if mode == "tf":
            trainResult.model = CRNNCrisprModel(trainResult.regression, device, trainResult.encoding.seqDim, trainResult.encoding.epiStart, trainResult.encoding.epiDim, trainResult.numBpWise, dynamic=True)
