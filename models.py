...
class CRNNCrisprModel(keras.Model):
    def __init__(self, regression, device, seqDim, epiStart, epiDim, numBpWise=0, verbose=False, isHPC=False, **kwargs):
    ...
    def call(self, x, training=None):
        if tf.shape(x).shape[0] < 3: x = tf.function(vecToMatEncoding)(x, numBpWise=self.numBpWise, seqDim=self.seqDim, setOR=True, bs=tf.shape(x)[0]) # decorating with tf.function makes Python functions run faster
...
class vecToMatEncoder(torch.nn.Module):
    def __init__(self, siamese, seqDim, interfaceMode, numBpWise, setOR):
    ...
    def forward(self, dataset):
        return vecToMatEncoding(dataset, seqDim=self.seqDim, single=self.interfaceMode, numBpWise=self.numBpWise, setOR=self.setOR)
